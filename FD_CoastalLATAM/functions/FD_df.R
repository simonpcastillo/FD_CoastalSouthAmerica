...
#' @param features: \code{numeric vector}. The number of featuresegories for each trait.Its length must be equal that the number of traits of each life-mode.
...
#' @details  An \emph{ecocode} is defined by a numeric sequence of traits as integers (e.g., 11211234); hence, the length of the \code{features} argument must be eight and its values cannot be numerically lower than the respective values in the ecocode's traits.
...
#'
#'
FD_df <- function(df, features){
  pacman::p_load(reshape2,stringr)
  if(length(features) != nchar(colnames(df)[1]))stop("Length of features must be equal that the number of traits for the life-modes.")
...
#  }
  ...
  traits<-as.data.frame(matrix(0, ncol = sum(features), nrow = nrow(data1)))
  colnames(traits)<-paste0(rep(letters[1:ntraits], features),sequence(features))
...
#      b <- as.numeric(data1[j,k])
 #     st <- sum(features[1:(k-1)]) - features[(k-1)]
